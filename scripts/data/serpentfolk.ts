...
type FlatTreasure = {
  img: string, name: string, desc: string, value: TreasureValue, quantity: number, size: ItemSizes, rarity: TreasureRarity
};

export function generateTreasure({
  img, name, desc, value, quantity, size, rarity,
}: FlatTreasure): Item<'treasure'> {
  return {
    img,
    name,
    system: {
